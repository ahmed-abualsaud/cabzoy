						...
						<tbody>
							...
							<?php if (is($companies, 'array')) foreach ($companies as $company) : ?>
								<tr>
									...
									<td>
										<a class="text-success" href="<?= !in_groups('creators') && (config('Settings')->enableHideSensitiveInfo || in_groups('demo-admins')) ? 'javascript:void(0)' : "tel://$company->company_mobile"; ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Call the user">
											+<?= !in_groups('creators') && (config('Settings')->enableHideSensitiveInfo || in_groups('demo-admins')) ? encrypt($company->company_mobile, 3) : $company->company_mobile; ?>
										</a>
									</td>
									<td>
										<a class="text-secondary d-block" href="<?= !in_groups('creators') && (config('Settings')->enableHideSensitiveInfo || in_groups('demo-admins')) ? 'javascript:void(0)' : "mailto://$company->company_email"; ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Send mail to user">
											<?= !in_groups('creators') && (config('Settings')->enableHideSensitiveInfo || in_groups('demo-admins')) ? encrypt($company->company_email, 2, true) : $company->company_email; ?>
										</a>
									</td>
